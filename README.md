...
| Tool | 说明 | 主要参数 |
|------|------|----------|
| `get_story_context` | 聚合写作上下文（写作规则、小说、章节、人物、关系、时间线、向量检索） | `novel_id`, `chapter_id?` |
| `get_writing_rules` | 获取给 LLM 使用的写作规则 | - |
...
> **写作规则**：首次启动会自动生成 `backend/data/writing_rules.md`。MCP 的 `get_story_context` 会自动返回 `writing_rules` 字段，也可单独调用 `get_writing_rules` 获取。
...
| `WRITING_RULES_FILE` | `data/writing_rules.md` | 提供给 MCP/LLM 的写作规则文件 |
